...
func (suite *OrderTestSuite) SetupTest() {
	...
	suite.service = NewBillingService(db, logger.Sugar(), cfg.CacheConfig, make(chan bool, 1), newGeoIpServiceTestOk(), "dev", "RUB")
...
func (suite *OrderTestSuite) TestOrder_ProcessFixedPackage_RegionFromRequest_Ok() {
	...
	assert.Equal(suite.T(), suite.project.FixedPackage["RU"].FixedPackage[0].Id, processor.checked.fixedPackage.Id)
	assert.Equal(suite.T(), suite.project.FixedPackage["RU"].FixedPackage[0].Name, processor.checked.fixedPackage.Name)
}
