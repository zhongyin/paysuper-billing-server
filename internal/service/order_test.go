...
func (suite *OrderTestSuite) TestOrder_GetOrder_Ok() {
	req1 := &billing.OrderCreateRequest{
		ProjectId:   suite.project.Id,
		Currency:    "RUB",
		Amount:      100,
	...
	req2 := &grpc.GetOrderRequest{
		MerchantId: suite.project.Merchant.Id,
		OrderId:    rsp1.Id,
	}
	rsp2 := &grpc.GetOrderResponse{}
	err = suite.service.GetOrder(context.TODO(), req2, rsp2)
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), pkg.ResponseStatusOk, rsp2.Status)
	assert.Empty(suite.T(), rsp2.Message)
	assert.Equal(suite.T(), rsp1.Id, rsp2.Item.Id)
}

func (suite *OrderTestSuite) TestOrder_GetOrder_NotFound_Error() {
